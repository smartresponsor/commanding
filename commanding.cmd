...
@echo off
setlocal EnableExtensions EnableDelayedExpansionREM --- Resolve repo dir ---
set "REPO_DIR="
if not "%~1"=="" (
...
)for %%I in ("%REPO_DIR%") do set "REPO_NAME=%%~nI"REM --- Default menu entry inside repo ---
set "COMMANDING_SH=.commanding/commanding.sh"
if not "%~2"=="" set "COMMANDING_SH=%~2"REM --- Detect Windows Terminal ---
set "WT=0"
where wt.exe >nul 2>nul && set "WT=1"REM --- Detect Git Bash (preferred) ---
set "BASH="
for %%P in (
...
set "WSL=0"
where wsl.exe >nul 2>nul && set "WSL=1"REM --- Command inside bash: start a single interactive bash that runs the menu ---
REM Notes:
...
set "BASH_CMD=if [ -f %COMMANDING_SH% ]; then exec bash -i %COMMANDING_SH%; else exec bash -i; fi"REM =========================
